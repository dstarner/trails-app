...
class Command(makemigrations.Command, BaseCommand):
    """Overrides the normal makemigrations command to include more validations."""

    def handle(self, *args, **options):
        """Include our custom logic to ensure extra validations are followed."""
        errors = {}
        ...
        for _, config in apps.app_configs.items():
            if not config.name.startswith(settings.LOCAL_APP_IMPORT_ROOT):
                continue
        ...
        if errors:
            ...
            self.stdout.write(self.style.ERROR(f'-- Model Validation Failed --\n\n{err_str}{nl}'))
            raise CommandError('Aborted')
        self.stdout.write(self.style.SUCCESS('All model Metas were successfully validated'))
